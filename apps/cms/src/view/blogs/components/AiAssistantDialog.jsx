...
/**
 * Dialog component for generating blog content using AI.
...
const AiAssistantDialog = ({ open, onClose, onGenerate, isPending }) => {
  ...
  /**
   * Resets the prompt text whenever the dialog is closed.
   */
  useEffect(() => {
    if (!open) {
      setPromptText("");
  ...
  return (
    <AppDialog
      open={open}
      onClose={onClose}
      dialogTitle="AI Writing Assistant"
      actions={[
        {
    ...
    >
      <Box sx={{ p: 2 }}>
        <AppInput
          ...
          placeholder="e.g. Write a technical blog about React performance optimization..."
        />
      </Box>
    </AppDialog>
  );
